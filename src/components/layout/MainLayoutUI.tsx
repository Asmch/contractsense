...
export function MainLayoutUI({ children, isDemo = false }: MainLayoutUIProps) {
  ...
  const navigation = [
    ...
    { name: "Contracts", href: isDemo ? "/demo" : "/contracts", icon: FileText },
    // { name: "Comparisons", href: isDemo ? "/demo/comparison" : "/comparisons", icon: GitCompare },
    ...
    <div className="min-h-screen bg-secondary/5 flex w-full relative">
      ...
      <div className="hidden lg:flex w-64 bg-white border-r border-border/50 flex-col h-screen sticky top-0 shrink-0">
        ...
        <div className="p-4 flex-1 flex flex-col gap-1">
          <div className="mb-4">
            <Link 
              href={isDemo ? "/demo" : "/contracts/upload"}
        ...
        <div className="p-4 border-t border-border/50">
          {isDemo ? (
            <Link 
              href="/"
              className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-muted-foreground hover:text-primary hover:bg-primary/5 transition-colors"
            >
      ...
      <div className="flex-1 flex flex-col min-h-screen overflow-x-hidden w-full lg:w-[calc(100%-16rem)]">
        <header className="h-16 bg-white/80 backdrop-blur-md border-b border-border/50 flex items-center justify-between px-4 lg:px-8 sticky top-0 z-40 w-full">
          <div className="flex items-center gap-4">
            ...
            {/* <h1 className="text-lg font-heading font-medium text-foreground truncate">Workspace</h1> */}
          </div>
          <div className="flex items-center gap-4">
            {!isDemo && (
              <Link 
        ...
        <main className="p-4 lg:p-8 max-w-7xl mx-auto w-full overflow-x-hidden relative">
          {children}
        </main>
      </div>
          ...
          <div className="relative flex flex-col w-64 max-w-[80vw] h-full bg-white shadow-2xl animate-in slide-in-from-left-full">
            ...
            <div className="p-4 border-t border-border/50">
              {isDemo ? (
                <Link 
                  href="/"
                  className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium text-muted-foreground hover:text-primary hover:bg-primary/5 transition-colors min-h-[44px]"
                >
                  <ArrowLeft className="w-4 h-4" />
                  Exit Tour
                </Link>
